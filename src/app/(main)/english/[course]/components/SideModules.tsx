...
const SideModules = ({modules, lastModuleId}: {modules: Module[], lastModuleId: number}) => {
           ...
           <div className="flex flex-col w-full overflow-y-auto max-h-[260px] " ref={scrollableDivRef}>
               {
                   modules?.map((moduleItem, index) => (
                       <div key={moduleItem.id} className={`w-full p-4 flex justify-between text-sm ${moduleItem.is_completed ? "text-[#7D7D7D]" : lastModuleId < moduleItem.id ? "cursor-not-allowed text-black" : "cursor-pointer text-black"}`}
                            onClick={() => lastModuleId >= moduleItem.id && router.push(`/english/${course}/${moduleItem.id}/reading`)}
                       >

                           <p className={`${moduleItem.id === Number(module) ? "text-[#7B68EE]" : moduleItem.is_completed ? "text-[#7D7D7D]" : "text-black"}` }>
                               {`Сабақ №${index + 1} - "${moduleItem.name}"`}
                           </p>
                           {
                               moduleItem.is_completed && (
                                   <div className={`flex gap-2`}>
                                       <span>{moduleItem.total_score.score} / {moduleItem.total_score.score}</span>
